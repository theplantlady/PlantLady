...
const JobPostCard = ({id, time, job_name, location, date_line, level, salary, link}) => {
    return (
        <li className='bg-secondary grid tab:grid-cols-[8%_25%_20%_20%_auto] tab:gap-y-0 gap-y-6 justify-between items-center mb-7.5 py-8 px-7.5 border border-primary'>
            <svg strokeWidth="1" className="h-[65px] w-20 relative -top-2  left-0 tl:text-6xl text-5xl inline-block font-extrabold leading-120 stroke-primary stroke-dasharray-1000 stroke-dashoffset-1000 animate-text-line-animation"><text x="0%" dominantBaseline="middle" y="70%">{id}</text></svg>
