...
const ServiceCard = ({id, service_name, service_desc, text_muted, link}) => {
    return (
        <div className='px-3 phx:px-0'>
            <OutlineSvgText text={id}/>
            <Link href={link} className={cn(`text-primary-foreground font-bold leading-135 tab:text-4xl text-3xl hover-underline ${text_muted}`)}><span>{service_name}</span></Link>
